...
def upload_background():
    ...
    try:
        ...
        if file.filename == '' or not file.content_type or not file.content_type.startswith('image/'):
...
# ── App Settings helpers (key/value store in DB) ──────────────────────────────
def _settings_init():
    try:
...
def get_theme():
    """Return current saved theme — public (no auth needed for styling)"""
    _settings_init()
    theme_json = _settings_get('ui_theme')
    if theme_json:
        try:
            import json as _j
            return jsonify({"ok": True, "theme": _j.loads(theme_json)})
        except:
            pass
    return jsonify({"ok": False, "theme": None})


@app.route('/api/bg-image', methods=['GET'])
def get_bg_image():
    """Return saved background image as base64 data URL"""
    _settings_init()
    img_data = _settings_get('bg_image')
    if img_data:
        return jsonify({"ok": True, "image": img_data})
    try:
        import base64 as b64m, os as _os
        if _os.path.exists('background.jpg'):
            with open('background.jpg', 'rb') as f:
                b64 = b64m.b64encode(f.read()).decode()
...
def save_bg_image():
    """Save background image (base64) + Groq analysis + save full theme"""
    _settings_init()
    try:
        import json as _j, base64 as b64m
        ...
        _settings_set('bg_image', data_url)

        # Overwrite background.jpg for legacy /background.jpg route
        try:
            raw = b64m.b64decode(img_b64)
            with open('background.jpg', 'wb') as f:
                f.write(raw)
        except:
            pass

        # Start theme from canvas-extracted vars
        theme = dict(css_vars) if css_vars else {}
        _settings_set('ui_theme', _j.dumps(theme))
        ...
        if GROQ_KEY:
            try:
                payload = {
                    ...
                    "messages": [{
                        ...
                        "content": [
                            ...
                            {"type": "text", "text": (
                                "Analyze this image carefully for colors, mood, and style. "
                                "Return ONLY a valid JSON object, absolutely no markdown or explanation:\n"
                                "{\n"
                                "  \"accent\": \"most vibrant/dominant color as CSS rgb()\",\n"
                                "  \"accent2\": \"slightly darker accent as CSS rgb()\",\n"
                                "  \"primary\": \"main background/base color as CSS rgb()\",\n"
                                "  \"secondary\": \"secondary accent as CSS rgb()\",\n"
                                "  \"gold\": \"warm highlight color as CSS rgb()\",\n"
                                "  \"green\": \"cool/nature accent color as CSS rgb()\",\n"
                                "  \"isDark\": true or false,\n"
                                "  \"radius\": \"12px for sharp/geometric, 20px for mixed, 28px for organic/natural\",\n"
                                "  \"btnRadius\": \"12px or 50% or 20px\",\n"
                                "  \"description\": \"Arabic phrase max 7 words describing mood\"\n"
                                "}"
                            )}
                        ]
                    }]
                ...
                if gr.ok:
                    raw_txt = gr.json()['choices'][0]['message']['content'].strip()
                    raw_txt = re.sub(r'```json|```', '', raw_txt).strip()
                    m = re.search(r'\{.*\}', raw_txt, re.DOTALL)
                    if m:
                        groq_result = _j.loads(m.group())
            except Exception as ge:
                print("Groq theme error:", ge)

        # ── Build full CSS variable set from Groq result ─────────────────────
        if groq_result:
            def _p(css):
                try:
            ...
            acc   = groq_result.get('accent', '')
            acc2  = groq_result.get('accent2', '')
            pri   = groq_result.get('primary', '')
            sec   = groq_result.get('secondary', '')
            gld   = groq_result.get('gold', '')
            grn   = groq_result.get('green', '')
            is_dk = groq_result.get('isDark', True)
            rad   = groq_result.get('radius', '20px')
            brd   = groq_result.get('btnRadius', '50%')
            desc  = groq_result.get('description', '')

            ap  = _p(acc)  if acc  else None
            a2p = _p(acc2) if acc2 else None
            pp  = _p(pri)  if pri  else None
            gp  = _p(gld)  if gld  else None
            gnp = _p(grn)  if grn  else None
            sp  = _p(sec)  if sec  else None
            ...
            if ap:
                t['--accent']       = acc
                t['--accent2']      = acc2 if acc2 else f"rgb({max(0,ap[0]-30)},{max(0,ap[1]-30)},{max(0,ap[2]-30)})"
                t['--accent-color'] = acc
                t['--accent-rgb']   = f"{ap[0]},{ap[1]},{ap[2]}"
                t['--accent-glow']  = f"rgba({ap[0]},{ap[1]},{ap[2]},0.28)"
                t['--primary-color']= acc
                t['--border']       = f"rgba({ap[0]},{ap[1]},{ap[2]},0.22)"
                t['--border2']      = f"rgba({ap[0]},{ap[1]},{ap[2]},0.10)"
            ...
            if gnp:
                t['--green']        = grn
                t['--green2']       = f"rgb({max(0,gnp[0]-25)},{max(0,gnp[1]-25)},{max(0,gnp[2]-25)})"
                t['--green-color']  = grn
            ...
            # Light/dark dependent
            if is_dk:
                t['--text']         = 'rgba(255,255,255,0.93)'
                t['--text2']        = 'rgba(200,195,210,0.85)'
                t['--text3']        = 'rgba(150,140,160,0.75)'
                t['--text-primary'] = 'rgba(255,255,255,0.93)'
                t['--card']         = 'rgba(255,255,255,0.08)'
                t['--card-bg']      = 'rgba(255,255,255,0.08)'
                t['--glass']        = 'rgba(255,255,255,0.06)'
                t['--bg-overlay']   = 'rgba(8,5,12,0.62)'
                t['--light-overlay']= 'rgba(8,5,12,0.16)'
                t['--shadow']       = 'rgba(0,0,0,0.45)'
            else:
                t['--text']         = 'rgba(20,10,30,0.93)'
                t['--text2']        = 'rgba(70,55,85,0.85)'
                t['--text3']        = 'rgba(120,105,135,0.75)'
                t['--text-primary'] = 'rgba(20,10,30,0.93)'
                t['--card']         = 'rgba(255,255,255,0.82)'
                t['--card-bg']      = 'rgba(255,255,255,0.82)'
                t['--glass']        = 'rgba(255,255,255,0.62)'
                t['--bg-overlay']   = 'rgba(235,230,245,0.38)'
                t['--light-overlay']= 'rgba(255,255,255,0.20)'
                t['--shadow']       = 'rgba(40,20,60,0.14)'

            # Background + nav from primary
            if pp:
                r, g, b = pp
                if is_dk:
                    t['--bg']     = f"rgb({max(6,r//7)},{max(6,g//7)},{max(6,b//7)})"
                    t['--bg2']    = f"rgb({max(10,r//5)},{max(10,g//5)},{max(10,b//5)})"
                    t['--nav-bg'] = f"rgba({max(6,r//5)},{max(6,g//5)},{max(6,b//5)},0.94)"
                    t['--orb1']   = f"rgba({min(255,r//2+40)},{min(255,g//2+20)},{min(255,b//2+60)},0.20)"
                    t['--orb2']   = f"rgba({min(255,g//2+40)},{min(255,b//2+40)},{min(255,r//2+20)},0.16)"
                    t['--orb3']   = f"rgba({min(255,b//2+60)},{min(255,r//2+30)},{min(255,g//2+40)},0.14)"
                else:
                    t['--bg']     = f"rgb({min(255,r+170)},{min(255,g+170)},{min(255,b+170)})"
                    t['--bg2']    = f"rgb({min(255,r+150)},{min(255,g+150)},{min(255,b+150)})"
                    t['--nav-bg'] = f"rgba({min(255,r+175)},{min(255,g+175)},{min(255,b+175)},0.94)"
                    t['--orb1']   = f"rgba({min(255,r+100)},{min(255,g+80)},{min(255,b+120)},0.30)"
                    t['--orb2']   = f"rgba({min(255,g+100)},{min(255,b+100)},{min(255,r+80)},0.25)"
                    t['--orb3']   = f"rgba({min(255,b+120)},{min(255,r+80)},{min(255,g+100)},0.22)"
        ...
        _settings_set('ui_theme', _j.dumps(theme))
        return jsonify({"ok": True, "theme": theme, "description": theme.get('_description', '')})

    except Exception as e:
        print("save_bg_image error:", e)
        return jsonify({"ok": False, "error": str(e)}), 500
